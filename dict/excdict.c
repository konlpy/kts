 ...
 *                                                                  *
 * excdict.c ( Korean POS Tagging System : Exception Dictionary )   *
 *                                                                  *
 *------------------------------------------------------------------*/
...
char *argv[] ; 
{
  ...
  JoinMark joinmark   ; /* Pre-Morph의 좌우 context          */
  ...
  if (argc != 3) {
	fprintf(stderr,"Usage : excdict Exception-Dictionary DBM-Dictionary\n") ; 
	exit(1) ; 
  }
  ...
  while (fscanf(dictPtr,"%s%s",red_entry,org_entry) != EOF) {
   ...
   joinmark.lmark = red_entry[0]           ; /* 예) +ㄴ$에서 + */
   joinmark.rmark = red_entry[inputsize-1] ; /* 예) +ㄴ$에서 $ */
